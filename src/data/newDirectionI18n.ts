    ...
    legal: {
      ...
      privacy: {
        ...
        sections: [
          ...
          { h: '2. Formularz kontaktowy', html: '<p>Formularz kontaktowy jest obsługiwany przez <strong>Netlify Forms</strong> — usługę mojego dostawcy hostingu Netlify. Po jego wysłaniu wprowadzone dane — imię, kanał kontaktu (Telegram lub email), typ projektu i treść wiadomości — są przechowywane przez Netlify wyłącznie po to, abym mógł odpowiedzieć na Twoje zapytanie.</p><p>Ukryte pole antyspamowe („honeypot”) pomaga blokować boty. Zamiast formularza możesz zawsze napisać do mnie bezpośrednio emailem lub przez Telegram.</p>' },
    ...
    legal: {
      ...
      privacy: {
        ...
        sections: [
          ...
          { h: '2. Форма обратной связи', html: '<p>Форма обрабатывается сервисом <strong>Netlify Forms</strong> моего хостинг-провайдера Netlify. После отправки введённые данные — имя, канал связи (Telegram или email), тип проекта и текст сообщения — хранятся у Netlify исключительно для того, чтобы я мог ответить на твой запрос.</p><p>Скрытое антиспам-поле («honeypot») помогает блокировать ботов. Вместо формы ты всегда можешь написать мне напрямую по email или в Telegram.</p>' },
    ...
    legal: {
      ...
      privacy: {
        ...
        sections: [
          ...
          { h: '2. Contact form', html: '<p>The contact form is processed by <strong>Netlify Forms</strong>, a service of my hosting provider Netlify. When you submit it, the data you enter — your name, contact channel (Telegram or email), project type and message — is stored by Netlify solely so that I can reply to your enquiry.</p><p>A hidden anti-spam field (“honeypot”) helps block automated bots. Instead of the form you can always contact me directly by email or Telegram.</p>' },
    ...
    legal: {
      ...
      privacy: {
        ...
        sections: [
          ...
          { h: '2. Форма зворотного зв’язку', html: '<p>Форма обробляється сервісом <strong>Netlify Forms</strong> мого хостинг-провайдера Netlify. Після надсилання введені дані — ім’я, канал зв’язку (Telegram або email), тип проєкту й текст повідомлення — зберігаються в Netlify виключно для того, щоб я міг відповісти на твій запит.</p><p>Приховане антиспам-поле («honeypot») допомагає блокувати ботів. Замість форми ти завжди можеш написати мені напряму на email або в Telegram.</p>' },
